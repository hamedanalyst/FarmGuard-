...
// Legacy Registrar removed - use embedding v2 FlutterPlugin lifecycle
...
public class PyTorchMobilePlugin implements FlutterPlugin, MethodCallHandler {
  ...
  public void onAttachedToEngine(@NonNull FlutterPluginBinding flutterPluginBinding) {
    ...
    // Load native libraries here (avoid throwing during class initialization)
    Log.i("PyTorchMobile", "onAttachedToEngine invoked");
    try {
      ...
      Log.i("PyTorchMobile", "Native libraries loaded successfully");
    } catch (Throwable t) {
      nativeLibrariesLoaded = false;
      Log.e("PyTorchMobile", "Failed to load native libraries", t);
    }
  }
    ...
    switch (call.method){
      case "loadModel":
        if (!nativeLibrariesLoaded) {
          ...
          Log.e("PyTorchMobile", "Native libraries not loaded, cannot load model: " + assetPath);
          result.error("NATIVE_LIB_LOAD_FAILED", "Native libraries not loaded: see logcat for details", null);
          break;
        }
        ...
        try {
          ...
          String assetPath = call.argument("assetPath");
          Log.i("PyTorchMobile", "loadModel called with absPath=" + absPath + " assetPath=" + assetPath);
          modules.add(Module.load(absPath));
          Log.i("PyTorchMobile", "Module.load succeeded for " + absPath);
          result.success(modules.size() - 1);
        } catch (Exception e) {
          String assetPath = call.argument("assetPath");
          Log.e("PyTorchMobile", assetPath + " is not a proper model", e);
          // Return an error to the Dart side so the caller doesn't hang waiting
          // for a result when loading the model fails.
          result.error("LOAD_MODEL_FAILED", assetPath + " is not a proper model: " + e.getMessage(), null);
        }
        break;
      case "predict":
        Module module = null;
      ...
      case "predictImage":
        ...
        try {
          ...
          imageModule = (Module) modules.get(index);
